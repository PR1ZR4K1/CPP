...
// private member function to compute depth of tree rooted in t
int depth(BinaryNode* t) const {
    if (t == nullptr) {  // empty tree has depth 0
        return 0;
    } else if (t->left == nullptr && t->right == nullptr) {  // leaf node has depth 1
        return 1;
    } else {  // non-empty tree with at least one child
        int left_depth = depth(t->left);
        int right_depth = depth(t->right);
        return 1 + std::max(left_depth, right_depth);  // depth is 1 + depth of deeper subtree
    }
}
